...
\marginnote{\textbf{\small You need to learn file I/O, string processing, 
...
\begin{itemize}
\item We create an empty list \textbf{P} that will keep our path, and
an empty list \textbf{V} to keep visited nodes.
\item We create a list \textbf{Q} that will keep a queue of
nodes we need to check. In the beginning the list contains
only starting node.
...
\textbf{P} and all neighbors of the current node to \textbf{Q}.
\item We continue the main loop.
\end{itemize}

Here is Python code:

\begin{lstlisting}[language=Python,style=codelst]{Python: Breadth first search}
def BFS(graph,P,start,goal):
    # initializing queue
    Q = [start]
    # initalaizing path and list of visited nodes
    P = []
